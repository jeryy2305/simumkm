...
export default function RequestProdukAdmin() {
    ...
    const [detailView, setDetailView] = useState<"request" | "participants">("request");
    ...
    const getRequestStatusLabel = (request: ProductRequest) => {
        if (request.status === "open" && request.offers?.some((offer) => offer.status === "pending")) {
            return "Menunggu Tester";
        }
    ...
    const getRequestStatusClasses = (request: ProductRequest) => {
        ...
        return getStatusClasses(request.status);
    };
    ...
    const handleOpenParticipants = async (id: number) => {
        setIsDetailModalOpen(true);
        setDetailView("participants");
        setDetailLoading(true);
        setDetailError(null);
        setSelectedRequest(null);
        ...
        try {
            const response = await authFetch(`${API_URL}/api/product-requests/${id}`);
            if (!response.ok) throw new Error("Gagal memuat peserta tester");
            const data = await parseJson<ProductRequestDetail>(response);
            setSelectedRequest(data);
        } catch (err: unknown) {
            setDetailError(err instanceof Error ? err.message : "Terjadi kesalahan saat memuat peserta tester");
        } finally {
            setDetailLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setFormData({ name: "", category: "Makanan", quantity: 1, reference_price: "", partner_profit: "", tester_delivery_date: "", purpose: "Mencari UMKM Penyedia Stok Produk" });
    ...
    return (
        <div className="space-y-6 pb-24 font-sans text-black">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
                <div>
                    <h1 className="text-3xl font-extrabold text-black mb-2">Request Produk</h1>
                    <p className="text-black text-sm md:text-base">Buat dan kelola permintaan produk untuk UMKM mengambil dan membuat produk.</p>
                </div>
                <div className="flex flex-wrap items-center gap-3">
                    <button
                        type="button"
                        onClick={refreshRequests}
                        className="inline-flex items-center justify-center gap-2 px-5 py-3 bg-gray-100 hover:bg-gray-200 text-black font-bold rounded-lg border border-gray-200 cursor-pointer"
                    >
                        ...
                        <table className="w-full text-left border-separate border-spacing-0">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {filtered.map((item, index) => (
                                    <tr key={item.id} className="transition-colors group align-top hover:bg-blue-50/40">
                                        ...
                                        <td className="py-3 px-3 align-top">
                                            <span className={`inline-flex items-center rounded-lg border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${getRequestStatusClasses(item)}`}>
                                        ...
                                        <td className="py-3 px-3 align-top">
                                            {item.offers?.length ? (
                                                <button
                                                    type="button"
                                                    className="cursor-pointer rounded-xl bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 transition-colors hover:bg-blue-600 hover:text-white"
                                                    onClick={() => void handleOpenParticipants(item.id)}
                                                >
                                                    Lihat {item.offers.length} peserta
                                                </button>
                                            ) : (
                                                <span className="text-sm font-semibold text-gray-500">Belum</span>
                                            )}
                                        </td>
                                        <td className="py-3 px-3 text-right align-top">
                                            <div className="flex flex-col sm:flex-row items-end justify-end gap-2">
                                                <button
                                                    type="button"
                                                    className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-blue-50 text-blue-700 transition-all hover:bg-blue-600 hover:text-white cursor-pointer"
                                                    onClick={() => void handleOpenDetail(item.id)}
                                                    title="Lihat detail request"
                                                    aria-label="Lihat detail request"
                                                >
                                                    <Eye size={16} />
                                                </button>
                                                <button
            ...
            <Modal
                ...
                title={detailView === "participants" ? "Peserta Tester" : "Detail Request Produk"}
            >
                {detailLoading ? (
                    ...
                    <div className="space-y-5">
                        <div className="rounded-3xl border border-blue-100 bg-blue-50/70 p-5">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                ...
                                <span className={`inline-flex items-center rounded-lg border px-3 py-1 text-[11px] font-bold uppercase tracking-wider ${getRequestStatusClasses(selectedRequest)}`}>
                        ...
                        {detailView === "request" && (
                        <div className="grid gap-4 rounded-3xl border border-gray-100 bg-gray-50/50 p-5 md:grid-cols-2">
                            ...
                            <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                                ...
                                <p className="mt-2 text-sm font-semibold text-gray-800">{formatDepartureDate(selectedRequest.tester_delivery_date)}</p>
                            </div>
                            <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
                        ...
                        <div className="rounded-3xl border border-gray-100 bg-gray-50 p-5">
                            ...
                            {selectedRequest.offers?.length ? (
                                <div className="mt-4 space-y-3">
                                    {[...selectedRequest.offers].sort((firstOffer, secondOffer) => secondOffer.id - firstOffer.id).map((offer) => (
                                        <div key={offer.id} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-colors hover:border-blue-200">
                                            <div className="flex flex-wrap items-start justify-between gap-3">
                                                ...
                                                {offer.status === "approved" && (
                                                    <span className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-bold uppercase text-emerald-700">
                                                        Disetujui
                                                    </span>
                                                )}
                                                {offer.status === "rejected" && (
                                                    <span className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1 text-[11px] font-bold uppercase text-rose-700">
                                                        Ditolak
                                                    </span>
                                                )}
                                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    value={formData.tester_delivery_date}
                                    onChange={(e) => setFormData({ ...formData, tester_delivery_date: e.target.value })}
                                />
                            </div>
